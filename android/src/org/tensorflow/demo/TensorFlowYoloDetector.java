    ...
    for (int y = 0; y < gridHeight; ++y) {
      for (int x = 0; x < gridWidth; ++x) {
        for (int b = 0; b < NUM_BOXES_PER_BLOCK; ++b) {
          ...
          final RectF rect = // 이 부분이 좌표 (left, top, right, bottom)
              new RectF(
                  Math.max(0, xPos - w / 2),
                  Math.max(0, yPos - h / 2),
          ...
          if (confidenceInClass > 0.01) {
            // LOGGER.i(
            //    "%s (%d) %f %s", LABELS[detectedClass], detectedClass, confidenceInClass, rect); // pill (0) 0.010058 RectF(0.0, 233.86008, 63.137417, 374.41336)
